...
En premier lieu, pour illustrer que ce fichier fonctionne de manière identique à précédemment, faisons un filtre à partir des variables de cette base (les géomaticiens parleraient de filtre attributaire). Récupérons exclusivement les établissements d'enseignement dans Aubervilliers:
...
leaflet(etab_enseignement_auber) %>%
  addTiles() %>%
  addMarkers(popup = ~paste0(NOMRS, "<br>", "Type d'établissement: ", TYPEQU)) %>%
